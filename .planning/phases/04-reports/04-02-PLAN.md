...
must_haves:
  truths:
    - "User sees console report showing transpilation status"
    - "User sees shortfall analysis with specific feature gaps listed"
    - "Shortfalls are categorized by cause with color coding"
  artifacts:
    - path: "src/types/index.ts"
      provides: "Extended TranspileResult with opencode and transformedArtifacts"
      exports: ["TranspileResult"]
      min_lines: 250
  ...
  key_links:
    ...
    - from: "src/lib/transpilation/reporter.ts"
      to: "TranspileResult.transformedArtifacts"
      via: "reads artifact metadata for status sections"
      pattern: "result\\.transformedArtifacts"
---
...
Output: Extended TranspileResult with artifact metadata; reporter.ts module that consumes TranspileResult and returns formatted console output with sections, colors, and categorized shortfalls.
</objective>
...
<task type="auto">
  <name>Task 1: Extend TranspileResult with artifact metadata</name>
  <files>src/types/index.ts</files>
  <action>
Extend the TranspileResult interface to include artifact metadata for reporting:

Add two optional fields to TranspileResult:
...
export interface TranspileResult {
  ...
  transformedArtifacts?: {
    commands: string[];  // command names like "/gsd:plan-phase"
...
**Why this approach:**
- Reporter needs artifact lists to construct status sections
- OpenCodeConfig contains full config for markdown export (Plan 03)
- transformedArtifacts provides simple lists for iteration
- Both are optional - orchestrator can provide them if available

Update JSDoc to explain these fields are populated by orchestrator for reporting purposes.
  </action>
  <verify>
Run: npm run build

Check TypeScript compilation succeeds. Check TranspileResult export includes opencode and transformedArtifacts fields.
  </verify>
  <done>
TranspileResult interface includes opencode and transformedArtifacts fields. TypeScript compilation succeeds with no errors.
  </done>
</task>

<task type="auto">
  <name>Task 2: Create reporter module with types and core structure</name>
...
2. **Types:**
   ...
   export interface FormattedReport {
     ...
     markdown: string;       // Markdown version (Phase 4 Plan 3)
...
<task type="auto">
  <name>Task 3: Implement artifact sections formatting</name>
  <files>src/lib/transpilation/reporter.ts</files>
  <action>
...
**Logic:**
1. Read artifact lists from `result.transformedArtifacts` (commands[], agents[], models[])
2. For each artifact:
   - Determine status: Success (no gaps matching this artifact), Partial (approximations), Failed (errors), Warning (warnings)
   - Match gaps to artifact using sourceFile field (compare gap.sourceFile against artifact name)
   - Format with appropriate symbol and color
   - Show target path (relative to config dir), and status details
...
**Simplified approach (no source paths):**
Since gaps.sourceFile contains the source file (like "commands.xml"), matching to individual artifacts requires parsing or inference.
Instead: Show artifact name, target, and generic status based on whether there are gaps at all.

**Fallback if transformedArtifacts missing:**
Show summary only: "N artifacts transformed (check manifest for details)"
...
Check formatArtifactSections() function exists and compiles. Manually test with mock TranspileResult in Node REPL to verify output format matches specification.
  ...
  <done>
formatArtifactSections() formats commands, agents, models with status symbols and target paths. Function compiles and produces expected console output.
  </done>
</task>

<task type="auto">
  <name>Task 4: Implement shortfall section and summary</name>
  <files>src/lib/transpilation/reporter.ts</files>
  <action>
...
5. Each gap shows: field/feature, reason, suggestion, source file (from gap.sourceFile)
6. **No deduplication** - list each gap individually per requirements

7. **Summary:** Calculate totals from gaps and artifact metadata
   - If transformedArtifacts available: use counts from there
   - Otherwise: estimate from gaps ("N artifacts with M gaps")
8. Show percentages for success/partial/failed (if artifact data available)
...
1. TranspileResult extended with opencode and transformedArtifacts fields
2. reporter.ts exports generateReport(), FormattedReport, ReportSummary
3. Console output includes artifact sections (if data available), shortfall section, summary
4. Colors applied via picocolors (red, yellow, green, blue, bold, dim)
5. Shortfall counts match actual gap lists
6. Quiet mode respected (minimal output)
7. TypeScript compilation succeeds
8. Tests verify formatting logic
</verification>

<success_criteria>
## Measurable Completion

- [ ] TranspileResult includes opencode and transformedArtifacts fields
- [ ] reporter.ts module exists with all exports
- [ ] generateReport() returns FormattedReport with formatted console output
- [ ] Console output includes: Shortfalls and Summary sections (artifacts optional)
- [ ] Shortfalls categorized by unsupported (red), platform (yellow), missing-dep (blue)
- [ ] Summary shows gap counts by category
...
- TranspileResult extensions (opencode, transformedArtifacts)
