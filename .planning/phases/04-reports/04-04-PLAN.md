...
return {
  ...
  transformedArtifacts: {
    commands: Object.keys(transformResult.opencode.commands ?? {}).length > 0
      ? transformResult.opencode.commands.map(c => c.name)
      : [],
...
Note: OpenCodeConfig uses arrays (agents, commands, models), not objects with keys. Extract names using .map() not Object.keys().
